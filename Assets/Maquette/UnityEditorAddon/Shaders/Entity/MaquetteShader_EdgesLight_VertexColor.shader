Shader "Maquette/DefaultEntity/EdgesLight_VertexColor"
{
	Properties
	{
		...
		SubShader
		{
			Pass 
			{
				...
				min16float4 frag(v2f i) : COLOR
				{
					...
					col.rgb += (min16float3)i.color.rgb * (min16float4)UNITY_ACCESS_INSTANCED_PROP(Props, _Color) * smoothstep((min16float)_LightSoftness, (min16float)0, dotProduct);
